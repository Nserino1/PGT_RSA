...
This script pulls motion parameters, aCompCor, and framewise displacement from fMRIPrep, then adds the rejected ICA components from tedana. It excludes confounds like cosine drift because nilearn's FirstLevelModel handles that automatically. The result is a streamlined confound file optimized for LSS beta extraction—one file per run that contains all the noise regressors we need., we must pull the files we want to make confound files ADD LATER NICOLE
